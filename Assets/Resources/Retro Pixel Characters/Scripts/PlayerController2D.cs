	...
	void Update()
	{
		...
		if (moveEnabled == true)
		{
			...
			if (Input.GetAxisRaw("Horizontal") > moveSense || Input.GetAxisRaw("Horizontal") < (-moveSense) || Input.GetAxisRaw("Vertical") > moveSense || Input.GetAxisRaw("Vertical") < (-moveSense))
			{
				...
				//Tell the animator which directional animation to display. 1 = down, 2 = right, 3 = left and 4 = up.
				if (moveVector.y < -0.01f)
				{
			...
			{
				//If there's no input, set the state to stand again and change Animator's walkDir value to 0 (standing).
